...
def latency(
    ...
    try:
        ...
        def instrumentation(info: Info) -> None:
            current_trace_id = get_current_traceid()
            if label_names:
                label_values = [
                ...
                METRIC.labels(*label_values).observe(info.modified_duration, exemplar={'trace_id':current_trace_id})
            else:
                METRIC.observe(info.modified_duration, exemplar={'trace_id':current_trace_id})
    ...
    try:
        ...
        def instrumentation(info: Info) -> None:
            ...
            if label_names:
                ...
                METRIC.labels(*label_values).inc(exemplar={'trace_id':current_trace_id})
            else:
                METRIC.inc(exemplar={'trace_id':current_trace_id})
...
def default(
    ...
    * `http_request_duration_seconds` (`handler`):
    ...
    try:
        ...
        def instrumentation(info: Info) -> None:
            current_trace_id = get_current_traceid()
            TOTAL.labels(info.method, info.modified_status, info.modified_handler).inc(exemplar={'trace_id':current_trace_id})
            ...
            if not should_only_respect_2xx_for_highr or info.modified_status.startswith(
                ...
                LATENCY_HIGHR.observe(info.modified_duration, exemplar={'trace_id':current_trace_id})

            LATENCY_LOWR.labels(info.modified_handler).observe(info.modified_duration, exemplar={'trace_id':current_trace_id})
